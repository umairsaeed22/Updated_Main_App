...
export const fetchSerials = async (pageNumber = 1, pagesSize = 10, status = 'NEW', location) => {
  const response = await apiRequest({
    endpoint: `/api/v1/Serial/Get?pageNumber=${pageNumber}&pagesSize=${pagesSize}&status=${status}&location=${location}`,
    method: 'GET',
    auth: true,
  });
...
export const getShiftsManagement = async (userId) => {
  const response = await apiRequest({
    endpoint: `/api/v1/Inventory/GetShifts?userId=${userId}`,
    method: 'GET',
    auth: true,
  });
